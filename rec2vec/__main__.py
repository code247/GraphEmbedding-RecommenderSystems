...
def process(args):
    ...
    model = Word2Vec(walks, min_count = 0, workers= args.workers)        
...
def predict_rating(model, nodedict, user, movie):
    ...
    movie_name = movie_node.name
    for i in range(1,6):
        rating_node = nodedict[movie_name + "_" + str(i)]
        sim = model.similarity(user_node.id, rating_node.id)
        if(maxsim < sim):
            maxsim = sim
            maxrating = i
    return maxrating
