...
def records_to_graph():
	...
	id = 0
	for user in users:
		n = Node(id, "u"+user, type = "user")
		n.rating = dict(ratings[user])
		nodelist.append(n)
		nodedict["u"+user] = n
		id += 1

	for m_id in movies.keys():
		n = Node(id, movies[m_id].name, type = "movie")
		nodelist.append(n)
		nodedict[m_id] = n
		id += 1
	...
	for m_id in movies.keys():
		for i in range(1,6):
			n = Node(id, movies[m_id].name + "_" + str(i), type = "rating")
			nodelist.append(n)
			nodedict[movies[m_id].name + "_" + str(i)] = n
			id += 1

	for d in directors:
		n = Node(id, d, type = "director")
		nodelist.append(n)
		nodedict[d] = n
		id += 1

	for a in actors:
		n = Node(id, a, type = "actor")
		nodelist.append(n)
		nodedict[a] = n
		id += 1

	for g in genres:
		n = Node(id, g, type = "genre")
		nodelist.append(n)
		nodedict[g] = n
		id += 1
	...
	# YOUR CODE HERE
	for user in users:
		user_node = nodedict["u"+user]
		for m_id in user_node.rating.keys():
			rating_node = nodedict[movies[m_id].name + "_" + user_node.rating[m_id]]
			user_node.neighbors.append(rating_node)
			rating_node.neighbors.append(user_node)

	for m_id in movies.keys():
		movie_node = nodedict[m_id]
		if(movies[m_id].director != None):
			director_node = nodedict[movies[m_id].director]
			director_node.neighbors.append(movie_node)
			movie_node.neighbors.append(director_node)
		gen = movies[m_id].genres
		act = movies[m_id].actors
		for g in gen:
		...
		for i in range(1,6):
			rating_node = nodedict[movie_node.name + "_" + str(i)]
